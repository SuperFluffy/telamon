...
fn build_candidate<'a>(space: SearchSpace<'a>,
                       ctx: &device::Context) -> explorer::Candidate<'a> {
...
/// to a valid value.
fn generate_tile_sizes(size: u32, max_tiling: u32, max_tile_dims: usize)
    -> (Vec<u32>, usize)
{
    let tile_sizes = (2..std::cmp::min(max_tiling, size/2)+1)
        ...
        .collect_vec();
    let num_tile_dims = tile_sizes.iter().take(max_tile_dims)
        .scan(1, |state, t| { *state *= t; Some(*state) })
...
fn limit_tile_size(dim: &LogicalDim, max_size: &[u32], builder: &mut Builder) {
    if let LogicalDim::Composite { dims, tiling_factors, .. } = dim {
